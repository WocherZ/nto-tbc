...
def decoder(filein, fileout, filelog):
    blocksize = 28 # значение в 2 раза большее чем в encoder.py
    enc_blocksize = 14 # значение из encoder.py
    s = list(filein.read(blocksize))
    ...
    while True:
        ...
        output_mesg = ''.join([chr(x) for x in decoded])
        print(output_mesg, len(output_mesg))
        output_mesg = output_mesg[0:int(len(output_mesg) - enc_blocksize):1]

        print('Output:', output_mesg, len(output_mesg))

        byte_array = bytearray(output_mesg, encoding="utf-8")
        print(byte_array)
...
# fileout = open('fileout.txt', 'wb')
# filelog = open('logs.txt', 'wb')
#
