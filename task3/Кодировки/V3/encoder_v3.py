...
def encoder(filein,fileout):
    ...
    while True:
        ...
        if not s:
            break
        encoded = fooRS.RSEncode(s, blocksize)
        print(encoded, len(encoded))
        #
        # s = list(encoded)
        # decoded_test = fooRS.RSDecode(s, blocksize)
        # print('Decoded:', decoded_test, len(decoded_test))
        #
        # output_mesg = ''.join([chr(x) for x in decoded_test])
        # print("Output message:", output_mesg, len(output_mesg))
        # output_mesg = output_mesg[0:int(len(output_mesg) - blocksize):1]
        #
        # byte1_array = bytearray(output_mesg, encoding="utf-8")
        # print("Output byte array:", byte1_array, len(byte1_array))
